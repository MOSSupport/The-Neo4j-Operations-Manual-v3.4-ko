...
초기 읽기 복제본 환경설정은 neo4j.conf을 통해서 비슷하게 코어 서버로 제공됩니다. 읽기 복제본은 클러스터 정족수에 관여하지 않기 떄문에 이 설정은 더 짧습니다. 이들은 검색 프로토콜을 실행하기위해 결합되는 일부 코어 서버의 주소를 알아야합니다. (참조: [섹션 4.2.1, 프로토콜 검색](./lifecycle.md). 작업이 완료되면, 초기 검색 읽기 복제본은 코어 서버를 사용하여 적절한 캐치 업을 선택합니다. (참조: [섹션 4.2.5, 캐치업 프로토콜](./lifecycle.md)). 
...
읽기 복제본은 또한 트랜잭션 폴링의 영향을 받습니다. 더 자세한 내용은 [멀티-데이터 센터 작동](./multi-clustering/config.md)을 참조하십시오.
